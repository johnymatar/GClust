...
int main(int argc, char* argv[]) {
	...
	if(myid==0){ //This is the main process
		...
		if(argc==1){
			...
			<<"usage: mpigclust -in [input fasta file] -out [output clustering file] -alignMode [alignment mode] -mdist [scoring matrix]\n"
			<<"   or: mpiexec -n [number of slave processes] gclust -in [input fasta file] -out [output clustering file] -alignMode [alignment mode] -mdist [scoring matrix]\n\n"
		...
		remove("matSimil.txt");
		remove("refs.txt");
		/*ifstream in("Clustering.txt", std::ios::in | std::ios::binary);
		ofstream out(fGroupes.c_str(), std::ios::out | std::ios::binary);
		out << in.rdbuf();
		remove("Clustering.txt");*/
		while(rename("Clustering.txt",fGroupes.c_str())<0);//file might still be locked for a while, so keep trying
